...
function Details({selectedCompany, setSelectedCompany}: Props) {
    ...
    return (
        ...
        >
            ...
            <Tabs variant={"fullWidth"} centered value={tab} onChange={(e, newValue) => setTab(newValue)}
                  aria-label="icon label tabs example">
                <Tab icon={<Info/>} label="General" value={"info"} sx={{textTransform: "capitalize"}}/>
                <Tab icon={<Groups/>} label="Employees" value={"employee"} sx={{textTransform: "capitalize"}}/>
                <Tab icon={<Public/>} label="Social Media" value={"media"} sx={{textTransform: "capitalize"}}/>
            </Tabs>
            {
                tab === "info" ? (
                    <InfoTab selectedCompany={selectedCompany}/>
                ) : null
            }
            {
                tab === "employee" ? (
                    <EmployeeTab selectedCompany={selectedCompany}/>
                ) : null
            }
            {
                tab === "media" ? (
                    <SocialTab selectedCompany={selectedCompany}/>
                ) : null
            }
        </Dialog>
    );
