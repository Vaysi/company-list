...
import {Google, Instagram, LinkedIn, MusicNote, Public, Twitter} from "@mui/icons-material";
import {
    CategoryScale,
    Chart as ChartJS,
    Filler,
        ...
        <Box>
            <Box sx={{display: "flex", justifyContent: "space-around", alignItems: "center",mt:3}}>
                { selectedCompany?.["LinkedIn - Followers"] ? (
                    <Button sx={{textTransform: "capitalize"}} variant="outlined" size={"small"} startIcon={<LinkedIn/>}>
                        {selectedCompany?.["LinkedIn - Followers"].toLocaleString('en-US')} Followers
                    </Button>
                ) : null}
                { selectedCompany?.["Twitter - Followers"] ? (
                    <Button sx={{textTransform: "capitalize"}} variant="outlined" color={"info"} size={"small"} startIcon={<Twitter/>}>
                        {selectedCompany?.["Twitter - Followers"].toLocaleString('en-US')} Followers
                    </Button>
                ) : null}
                { selectedCompany?.["Instagram - Followers"] ? (
                    <Button sx={{textTransform: "capitalize"}} variant="outlined" color={"warning"} size={"small"} startIcon={<Instagram/>}>
                        {selectedCompany?.["Instagram - Followers"].toLocaleString('en-US')} Followers
                    </Button>
                ) : null}
                { selectedCompany?.["Google Play - Reviews"] ? (
                    <Button sx={{textTransform: "capitalize"}} variant="outlined" color={"success"} size={"small"} startIcon={<Google/>}>
                        {selectedCompany?.["Google Play - Reviews"].toLocaleString('en-US')}  Reviews
                    </Button>
                ) : null}
                { selectedCompany?.["iTunes - Reviews"] ? (
                    <Button sx={{textTransform: "capitalize"}} variant="outlined" color={"primary"} size={"small"} startIcon={<MusicNote/>}>
