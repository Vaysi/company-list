...
function Filters({data, setFilteredData}: Props) {
    ...
    const regions = useMemo(() => {
        const out = data ? [...new Set(data.map(item => item["HQ Region"]))].filter(item => item) : [];
        return out.map((item, index) => ({label: item, id: index}));
    }, [data]);

    const [industry, setIndustry]: [I_autoComplete, any] = useState<I_autoComplete>(industryInitialState);
    ...
    const filterByIndustry = useCallback(
        () => {
            setFilteredData(industry.label === industryInitialState.label ? data : data?.filter(item => item.Industry === industry.label));
        },
    ...
    const filterByRegion = useCallback(
        () => {
            setFilteredData(region.label === regionInitialState.label ? data : data?.filter(item => item["HQ Region"] === region.label));
        },
    ...
    return (
        <Grid container sx={{px: 1}}>
            <Grid item xs={12} sm={6} md={3} sx={{pr: {xs: 0, sm: 1}}}>
                <FormControl variant={"outlined"} fullWidth={true} sx={{my: {xs: 1, sm: 1, md: 1, lg: 0}}}>
            ...
            <Grid item xs={12} sm={6} md={3} sx={{pr: {xs: 0, sm: 1}}}>
                <Autocomplete
                    ...
                    options={industries}
                    value={industry}
                    onChange={(event, newValue) => newValue ? setIndustry(newValue) : setIndustry(industryInitialState)}
            ...
            <Grid item xs={12} sm={6} md={3} sx={{pr: {xs: 0, sm: 1}}}>
                <Autocomplete
                    ...
                    options={regions}
                    value={region}
                    onChange={(event, newValue) => newValue ? setRegion(newValue) : setRegion(regionInitialState)}
